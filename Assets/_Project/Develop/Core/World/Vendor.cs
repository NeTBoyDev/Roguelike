...
public class Vendor : MonoBehaviour
{
    ...
    [SerializeField] private string[] _noMoneyPhrases = { "Not enough gold!", "YouТre short on coin!", "Come back with more gold!" };
    [SerializeField] private string[] _sellPhrases = { "Thanks for the goods!", "A fair trade!", "IТll take that off your hands!" };
    #endregion
    ...
    private void HandleSell(PointerEventData data, Item item, InventorySlot targetSlot)
    {
        ...
        else if (item.IsStackable && targetSlot.Item.Id == item.Id && targetSlot.Item.Rarity == item.Rarity)
        {
            ...
            {
                // —вап, если один из стаков полный
        ...
        else // ѕростой свап дл€ нестакаемых предметов
        {
            Item targetItem = targetSlot.Item;
            targetSlot.InitializeSlot(item);
            sourceSlot.InitializeSlot(targetItem);
