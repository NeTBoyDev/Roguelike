...
public class LevelGenerator : MonoBehaviour
{
    ...
    [field: MinValue(1), SerializeField] public int MaxRoomCount { get; private set; } = 1;
    [field: MinValue(0), SerializeField] public float StartSpawnDelay { get; private set; } = 1;
    [field: MinValue(0.001f), SerializeField] public float RoomSpawnDelay { get; private set; } = 0.1f;
    ...
    public async UniTask<LevelGenerator> GenerateFloor(int florCount = 1)
    {
        ...
        Debug.Log($"Генерация завершена. Пыталось создать: <color=cyan>{_currentRoomCount}</color> комнат. Успешно создано: <color=yellow>{SpawnedRooms.Count}</color>");
        return this;
    }
    ...
    {
        var randomStartRoom = RoomPrefabs.Where(r => r.Type == RoomType.StartRoom)
    ...
    {
        var randomLastRoom = RoomPrefabs.Where(r => r.Type == RoomType.LastRoom)
            .OrderBy(_ => UnityEngine.Random.value)
            .FirstOrDefault();
    ...
    private void RemoveRoom(Room currentRoom)
    {
        ...
        _currentRoomCount = Mathf.Clamp(_currentRoomCount--, SpawnedRooms.Count, MaxRoomCount);
    }
    ...
    public IEnumerable<Room> GetAllRandomBaseRooms()
        => RoomPrefabs.Where(r => r.Type == RoomType.BaseRoom).OrderBy(_ => UnityEngine.Random.value);
    public IEnumerable<Room> GetAllRandomCorridors()
        => RoomPrefabs.Where(r => r.Type == RoomType.Corridor).OrderBy(_ => UnityEngine.Random.value);
        ...
        foreach (var spawnedRoom in SpawnedRooms)
        {
            ...
            foreach (var roomCollider in room.RoomColliders)
            {
                foreach (var spawnedRoomCollider in spawnedRoom.RoomColliders)
                {
                    ...
                    {
                        Debug.Log($"Removed object = {spawnedRoom} ");
                        return true;
                    }
                }
            }
